...
|---|---|---:|---|---|---|---|
| `data/processed/base_grid_1deg.parquet` | Shared 1 deg Io longitude-latitude grid. | ~406 KB | Yes | No | Render persistent disk or v2 GitHub Release data asset | Regenerate with the grid preprocessing pipeline. |
| `data/processed/feature_matrix.parquet` | Main merged feature table for dashboard maps and older model diagnostics. | ~1.3 MB | Yes | No | Render persistent disk or v2 GitHub Release data asset | Regenerate after raw hotspot, geology, tidal, and power layers are available. |
...
| `data/processed/power_grid_1deg.parquet` | Estimated thermal power proxy mapped to the common 1 deg grid. | ~433 KB | Yes | No | Render persistent disk or v2 GitHub Release data asset | Regenerate from `data/raw/io_hotspot_power.csv` and related thermal proxy inputs. |
...
| `data/raw/io_hotspot_power.csv` | Estimated hotspot power proxy input. | ~76 KB | Needed for rebuild | No | v2 data archive or source catalogue URL | Restore before regenerating `power_grid_1deg.parquet`. |
